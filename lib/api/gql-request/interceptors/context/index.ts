...
export class GQLContext {
    ...
    async sendRequest(): Promise<GQLResponse> {
        ...
        const val = NativeRequestAdapter(this) as [query: string, variables?: unknown, requestHeaders?: HeadersInit | undefined];
        try {
            const res = await this.client.rawRequest.apply(this.client, val) as GQLResponse
